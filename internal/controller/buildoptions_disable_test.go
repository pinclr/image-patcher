...
func kanikoArgs(buildOpts omsv1alpha1.BuildOptions, buildCacheRepo, dedupRef string) []string {
	return kanikoArgsWithMirrors(buildOpts, buildCacheRepo, dedupRef, nil)
}

func kanikoArgsWithMirrors(buildOpts omsv1alpha1.BuildOptions, buildCacheRepo, dedupRef string, mirrors map[string]string) []string {
	...
	j := constructJob(cr, "tc-job", "tc-cm", "ns", "registry.local/app:v1", "kaniko:test",
		...
		corev1.ResourceRequirements{}, buildOpts, dedupRef, mirrors)
